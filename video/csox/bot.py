def generate_rss_xml_sitemap(urls):
    ...
    xml_sitemap += '<rss xmlns:dc="http://purl.org/dc/elements/1.1/" version="2.0" xml:base="https://cv.xebia.fr/">\n'
    xml_sitemap += '<channel>\n'
    xml_sitemap += '<title>cv.xebia.fr</title>\n'
    xml_sitemap += '<link>https://cv.xebia.fr/</link>\n'
...
def create_sitemaps(url_list, urls_per_sitemap):
    ...
    for i in range(num_sitemaps):
        ...
        with open(f'abavfr{i + 1}.xml', 'w') as f:
